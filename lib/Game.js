...
Game.create = function(io) {
  var platforms = [
    Platform.create([0, -9], [2500, 10]),
    Platform.create([100, 50], [100, 10])
  ]
...
Game.prototype.updatePlayerOnInput = function(id, data) {
  ...
  this.players.get(id).updateOnInput(
      ...
      function(position, orientation) {
        console.log(id, position, orientation);
        // context.projectiles.push(Projectile.create(id, position, orientation));
      });
};
